...
def ensure_test_data():
    ...
    for category_key, category_data in degree_seed.items():
        category = DegreeCategory.query.filter_by(key=category_key).first()
        ...
        for option_name in category_data["options"]:
            existing_option = DegreeOption.query.filter_by(name=option_name).first()
            ...
            if not existing_option:
                ...
                db.session.add(degree_option)
            else:
                existing_option.category_id = category.id
